...
with tab6:
    ...
    if prod_df.empty:
        ...
        # 强制将分组列转换为字符串，并处理列表类型
        for col in ["style_code", "master_category", "image_url"]:
            if col in prod_df.columns:
                # 如果元素是列表，取第一个元素
                prod_df[col] = prod_df[col].apply(lambda x: x[0] if isinstance(x, list) else x)
                # 转换为字符串
                prod_df[col] = prod_df[col].astype(str)
                # 将 'nan' 字符串替换为 None
                prod_df[col] = prod_df[col].replace('nan', None)
                # 将空字符串替换为 None
                prod_df[col] = prod_df[col].replace('', None)
        ...
        if filtered.empty:
            ...
            try:
                ...
                st.error(f"分组聚合失败，降级为仅按货号分组: {group_err}")
            ...
            # 饼图（如果存在分类且没有降级）
            if "master_category" in grouped.columns and grouped["master_category"].notna().any():
...
with tab_debug:
    ...
    if supabase is None:
        ...
        try:
            ...
            st.error(f"查询 product_sales 表失败: {e}")# -*- coding: utf-8 -*-
"""
订单业绩统计工具 - 最终稳定版（彻底修复分组错误）
访问密码：94949468
"""
