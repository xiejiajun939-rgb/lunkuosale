    ...
    with st.form("login_form"):
        ...
        if submitted:
            if username in USERS and USERS[username]["password"] == password:
                ...
                # 注意：普通用户默认后缀不变，但 admin 登录后可通过侧边栏切换
...
with st.sidebar:
    ...
    if st.session_state.role == "admin":
        ...
        # 找出当前选中的选项
        current_source = [k for k, v in source_options.items() if v == current_suffix]
        if not current_source:
            current_source = "非直播数据"
        else:
        ...
        new_suffix = source_options[data_source]
