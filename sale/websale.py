# -*- coding: utf-8 -*-
"""
订单业绩统计工具 - Streamlit 网页版（带 Supabase 持久化）
支持上传订单文件，自动汇总每日及累计业绩，对接目标达成率，并保存每日业绩到云端数据库
运行命令：streamlit run websale.py
"""
...
# ========== Supabase 连接（使用 st.connection） ==========
try:
    from st_supabase_connection import SupabaseConnection
    supabase_available = True
except ImportError:
    supabase_available = False
    st.warning("未安装 st_supabase_connection，历史数据功能将不可用。请运行: pip install st_supabase_connection")

# 初始化 Supabase 客户端
if supabase_available and "supabase" not in st.session_state:
    try:
...
def save_to_supabase(df_all):
    ...
    if not st.session_state.get("supabase_ready", False):
        return
...
def process_order_file(uploaded_file):
    ...
    try:
        ...
        # 保存到 Supabase
        if st.session_state.get("supabase_ready", False):
...
with tab5:
    ...
    if not history_df.empty:
        ...
        st.info("暂无历史数据，请先上传订单文件并确保 Supabase 配置正确")
