...
with tabs[tab_index_product]:
    # ----- 弹窗状态管理（关键修复）-----
    # 仅在非详情点击导致的 rerun 时清除弹窗状态
    if st.session_state.get("detail_clicked", False):
        # 本次刷新由查看详情按钮触发，保留弹窗状态，并清除标记
        st.session_state.detail_clicked = False
    else:
        # 其他任何原因（筛选、排序、翻页、刷新等）导致的 rerun，清除弹窗
        ...
        if filtered.empty:
            ...
            for idx, row in page_df.iterrows():
                ...
                if col8.button("📊 查看详情", key=f"detail_btn_{row['货号']}_{idx}"):
                    ...
                    st.session_state.detail_clicked = True   # 标记本次为详情点击导致的刷新
                    st.rerun()
