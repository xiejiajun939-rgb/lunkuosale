...
with tabs[tab_index_product]:
    ...
    if prod_df.empty:
        ...
        with col_shop:
            # 需要先获取所有店铺以便展示选项，但此时 filtered 还未计算，可以先从 prod_df 获取（未经过日期筛选）
        ...
        # 构建当前筛选条件的唯一标识
        filter_key = f"{start_date}_{end_date}_{selected_platform}_{selected_shops}_{style_codes_input}_{selected_brand}_{selected_anchors}"
        ...
        # 应用筛选（基于 prod_df，而不是 filtered，因为我们还没过滤）
        # 先按日期范围筛选
