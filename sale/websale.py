# -*- coding: utf-8 -*-
"""
订单业绩统计工具 - 完整最终版（管理员可切换数据源：非直播/直播/全部，独立数据库表）
修复：上传文件时同日期数据覆盖而非累加
...
def save_daily_sales(records, suffix=None):
    if supabase is None or not records:
        return
    table_name = get_table_name("daily_sales", suffix)
    # 使用 upsert，基于 sale_date, shop_name 唯一键（若未创建约束，insert 可能重复）
...
def validate_order_data(df):
    """验证上传的订单数据格式，返回 (is_valid, error_message, cleaned_df)"""
    try:
        required = ["日期", "金额/时间", "备注"]
        missing_cols = [col for col in required if col not in df.columns]
        if missing_cols:
            return False, f"缺少必要列: {', '.join(missing_cols)}。请确保文件包含：日期、金额/时间、备注。", None
        df_valid = df.copy()
...
def process_uploaded_file(uploaded_file, suffix):
    try:
        ...
        # 5. 获取现有的 daily_sales 数据，并删除与新文件日期重合的所有旧记录
...
with st.sidebar:
    ...
    if st.session_state.role == "admin":
        ...
        if st.button("🗑️ 清除当前用户的目标记忆", key="clear_targets_btn"):
            clear_targets(st.session_state.table_suffix)
        if st.button("🔄 强制刷新所有数据", key="force_refresh_btn"):
            st.cache_data.clear()
            st.rerun()
        if st.button("🔁 重置为非直播数据", key="reset_to_normal_btn"):
            st.session_state.table_suffix = ""
            st.cache_data.clear()
            st.rerun()
        if st.button("🔄 从商品明细重建每日业绩", key="rebuild_daily_btn", help="根据当前商品销售表重新生成每日业绩表（会覆盖现有数据）"):
